...
fun List<MutableList<Char>>.parityFloodFillLimit(at: Coord, water: Char, spaces: List<Char>, limit: Int) {
    if (!this.validIndex(at)) return
    ...
    while (queue.isNotEmpty()) {
        ...
        if (this[next] in spaces && distance <= limit+1) {
            ...
            queue.addAll(next.getCardinalBorder().filter { this.validIndex(it) }.map { it to distance+1 })
        }
    }
}